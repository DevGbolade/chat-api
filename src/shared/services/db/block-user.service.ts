...
class BlockUserService {
  public async blockUser(userId: string, followerId: string): Promise<void> {
    UserModel.bulkWrite([
      {
        updateOne: {
          filter: { _id: userId as any, blocked: { $ne: new mongoose.Types.ObjectId(followerId) } },
          update: {
            $push: {
      ...
      {
        updateOne: {
          filter: { _id: followerId as any, blockedBy: { $ne: new mongoose.Types.ObjectId(userId) } },
          update: {
            $push: {
