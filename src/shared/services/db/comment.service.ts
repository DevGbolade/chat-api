...
class CommentService {
  public async addCommentToDB(commentData: ICommentJob): Promise<void> {
    const { postId, userTo, userFrom, comment } = commentData;
    ...
    if (response[2].notifications.comments && userFrom !== userTo) {
      ...
      emailQueue.addEmailJob('commentsEmail', { receiverEmail: response[2].email!, template: 'template', subject: 'Post notification' });
    }
  }
